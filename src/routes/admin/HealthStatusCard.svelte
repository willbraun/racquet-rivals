<script lang="ts">
	...
	function getStatus(error: string): 'success' | 'error' {
...
<div class="rounded-xl border border-gray-300 bg-white p-6 shadow-sm">
	...
	{#if healthData}
		{@const status = getStatus(healthData.error)}
		<div class="flex flex-col gap-3">
			<div class="flex items-center gap-3">
				<span class="text-lg font-medium sm:text-xl">Status:</span>
				<span
					class="rounded-full px-4 py-1 text-sm font-semibold sm:text-base {status === 'success'
			...
			{#if healthData.error}
				<div class="mt-2 flex flex-col gap-1">
					<p class="text-sm font-medium text-red-600 sm:text-base">Error Details:</p>
					<p
						class="overflow-x-auto rounded bg-red-50 p-3 font-mono text-sm break-words whitespace-pre-wrap text-red-900 sm:text-base"
					>
						{healthData.error}
					</p>
				</div>
