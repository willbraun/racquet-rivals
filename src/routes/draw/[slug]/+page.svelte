...
<Header color="bg-primary-50">
	<select
		class="select hover:bg-primary-100 -mt-1 h-full grow cursor-pointer bg-transparent py-0 text-xl font-bold whitespace-pre-wrap md:text-2xl"
								...
								{#if slot.name.trim()}
									...
									<div class="flex w-full items-center">
										{#if index === 0 && data.earlySlots.length > 0}
											<button
												class="p-2 text-xs text-stone-500"
												onclick={() => {
													earlyRoundsData.set({
														slot,
										...
										{/if}
										<p
											class={`${slot.name.length > 16 ? 'text-sm xl:text-lg' : 'text-lg'} w-full text-center leading-tight ${index > 0 ? 'mb-1' : ''}`}
