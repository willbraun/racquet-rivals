...
<table class="w-full overflow-hidden rounded-xl shadow-sm">
	...
	<tbody>
		{#each data.rankings.items as item, index (item.id)}
			<tr
				class={`grid min-h-16 grid-cols-5 items-center gap-2 px-2 text-start text-xl hover:cursor-pointer hover:brightness-105 md:min-h-20 md:text-3xl ${index % 2 ? 'bg-primary-50' : 'bg-primary-100'}`}
